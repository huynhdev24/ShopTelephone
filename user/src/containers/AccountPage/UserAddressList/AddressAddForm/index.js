...
function AddressAddForm(props) {
  ...
  const getDistrictList = async (provinceId = 0) => {
    try {
      const response = provinceListJSON.find((province) => province.id === provinceId)
      if (response) {
  ...
  const getWardStreetList = async (provinceId = 0, districtId) => {
    try {
      const response = districtList.find((district) => district.id === districtId)
      if (response) {
        setStreetList(response.streets);
  ...
  const onAddAddress = async (newAddress) => {
    try {
      ...
      const provinceName = provinceListJSON.find((provinceItem) => provinceItem.id == province).name
      const districtName = districtList.find((districtItem) => districtItem.id == district).name
      const wardName = wardList.find((wardItem) => wardItem.id == wards).name
      const sentData = { name, phone, address: { province: provinceName, district: districtName, wards: wardName, details} };
